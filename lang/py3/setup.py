...
def runs_from_source_dist():
    ...
    pkg_info_file_path = os.path.join(os.path.dirname(setup_file_path), "PKG-INFO")
...
def setup_sources():
    ...
    py_src_dir = os.path.join(py3_dir, "src", "main", "python")
    py_test_dir = os.path.join(py3_dir, "src", "test", "python")
    ...
    shutil.copy(src=os.path.join(root_dir, "README.txt"), dst=os.path.join(py3_dir, "README.txt"))

    # Read and copy Avro version:
    version_file_path = os.path.join(root_dir, "share", VERSION_FILE_NAME)
    with open(version_file_path, mode="rt", encoding="UTF-8") as f:
        avro_version = f.read().strip()
    shutil.copy(src=version_file_path, dst=os.path.join(py_src_dir, "avro", VERSION_FILE_NAME))

    # Copy necessary avsc files:
    avsc_file_path = os.path.join(root_dir, "share", "schemas",
                                  "org", "apache", "avro", "ipc", "HandshakeRequest.avsc")
    shutil.copy(src=avsc_file_path, dst=os.path.join(py_src_dir, "avro", "HandshakeRequest.avsc"))

    avsc_file_path = os.path.join(root_dir, "share", "schemas",
                                  "org", "apache", "avro", "ipc", "HandshakeResponse.avsc")
    shutil.copy(src=avsc_file_path, dst=os.path.join(py_src_dir, "avro", "HandshakeResponse.avsc"))

    avsc_file_path = os.path.join(root_dir, "share", "test", "schemas", "interop.avsc")
    shutil.copy(src=avsc_file_path, dst=os.path.join(py_test_dir, "tests", "interop.avsc"))

    # Make sure the avro shell script is executable:
    os.chmod(path=os.path.join(py3_dir, "scripts", "avro"), mode=0o777)
...
def read_version():
    ...
    version_file_path = os.path.join(install_dir, "src", "main", "python", "avro", VERSION_FILE_NAME)
...
def main():
    assert (sys.version_info[0] >= 3), ("Python version >= 3 required, got %r" % sys.version_info)

    if not runs_from_source_dist():
        setup_sources()

    avro_version = read_version()
    ...
    setup(
        ...
        description = "Unofficial Avro library for Python3.",
